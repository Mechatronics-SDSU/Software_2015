...
class GuiCreation:
    ...
    def guiSetup(self):
        ...
        setattr(window, 'quitFlag', False) #A my_main Python function that gives functions new attributes/variables
        ...
        commFrame.grid(row = 0, column = 0, ipadx = int(screenRes[0]/15.9), sticky = "w")
        ...
        #commandListBox.grid(row = 2, column = 0, rowspan = 7, columnspan = 4)
        
        command_value = Tkinter.StringVar()
        commandOptions = Tkinter.OptionMenu(commFrame, command_value, '')
        commandOptions.grid(row=2,column=0, columnspan = 2)
        commandOptions.bind("<Button-1>", lambda event, args=[window]: Refresh(event, args)) #This is to stop the gui from flickering
        
        setattr(window, "boardOptions", boardOptions) #This is so the COMfinder can update tab 3
        setattr(window, "commandOptions", commandOptions) #This is so the COMfinder can update tab 3
        setattr(window, "boardValue", board_value) #This is so the COMfinder can update tab 3  
        setattr(window, "commandValue", command_value) #This is so the COMfinder can update tab 3
        #setattr(window, "commandListBox", commandListBox) #This is so the COMfinder can update tab 3
        
            
        addCommand = Tkinter.Button(commFrame, text = "Add Command", font=("TkDefaultFont", int(round(screenRes[0]/176.667))), command = lambda: gui_communication.DvlControl(window).commInterface())
        addCommand.grid(row = 3, column = 0, pady = int(screenRes[1]/200), columnspan = 2)
        
        loadScript = Tkinter.Button(commFrame, text = "Load", font=("TkDefaultFont", int(round(screenRes[0]/176.667))), command = lambda: gui_communication.TcbControl(window).commInterface())
        loadScript.grid(row = 4, column = 0, pady = int(20))
        ...
        runButton = Tkinter.Button(commFrame, text = "Run", font=("TkDefaultFont", int(round(screenRes[0]/176.667))), command = lambda: gui_communication.Sparton_AHRS(window).commInterface())
        runButton.grid(row = 5, column = 0, pady = int(screenRes[1]/200), columnspan = 2)
        
        #scriptLabel = Tkinter.Label(tab3, text = "               Script", font=("TkDefaultFont", int(round(screenRes[0]/176.667))))
        #scriptLabel.grid(row = 0, column = 0)
        ...
        outputFrame = Tkinter.Frame(tab3)
        outputFrame.grid(row = 0, column = 3)
        outBar = Tkinter.Scrollbar(outputFrame)
        outBar.grid(row = 0, column = 2, rowspan = 10, sticky='ns')
        scriptOut = Tkinter.Text(outputFrame, yscrollcommand=outBar.set, borderwidth = 7, width = int(screenRes[0]/31.8), height = int(screenRes[1]/39.5), wrap="word")
        outBar.config(command=scriptOut.yview)
        scriptOut.grid(row = 0, column = 1, rowspan = 10, sticky = "nesw")
