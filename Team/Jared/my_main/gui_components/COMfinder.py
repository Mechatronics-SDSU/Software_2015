...
class UpdateBoard:
    ...
    def update(self, window):
        board = window.boardValue.get()
        menu = window.commandOptions["menu"]
        menu.delete(0, "end")
        if board == "Sparton":
            spartonList = self.log.getParameters("Sparton")
            spartonOptions = spartonList.Sparton
            for option in spartonOptions:
                menu.add_command(label=option)
        if board == "TCB":
            TCBList = self.log.getParameters("TCB")
            TCBOptions = TCBList.TCB
            for option in TCBOptions:
                menu.add_command(label=option)
        if board == "DVL":
            DVLList = self.log.getParameters("DVL")
            DVLOptions = DVLList.DVL
            for option in DVLOptions:
                menu.add_command(label=option)
        print board
