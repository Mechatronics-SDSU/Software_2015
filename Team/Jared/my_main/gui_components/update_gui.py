...
class UpdateGUI():
    ...
    def __imageProcessing__(self, frontImg, bottomImg, frontRawImgLabel, bottomRawImgLabel, frontProcessedImgLabel, bottomProcessedImgLabel, filterScales):
        ...
        maskFrontImg = cv2.inRange(HSVFrontImg, lowerBound , upperBound) #Black and white image
        epsilon = filterScales[8].get()
        if epsilon != 0:
            blurFrontImg = cv2.blur(maskFrontImg, (2,2))
            ret,thresh = cv2.threshold(blurFrontImg,127,255,0)
            flag, contours, h = cv2.findContours(thresh,3,2)
            squares_cont = []
            minCurves = filterScales[9].get()
            maxCurves = filterScales[10].get()
            for n, cnt in enumerate(contours):
                approx = cv2.approxPolyDP(cnt,epsilon*0.001*cv2.arcLength(cnt,True),True)#0.17 only gets the squares with nothing
                if len(approx)>=minCurves and len(approx)<=maxCurves:
                    squares_cont.append([n,cnt])
            if squares_cont != None:
                for i, cnt in squares_cont:
                    if h[0][i][3] > -1:
                        cv2.drawContours(morphologyFrontImg,[cnt],0,(255,255,0),3)
                    else:
                        cv2.drawContours(morphologyFrontImg,[cnt],0,(255,0,0),4)
        ...
        return trackBoxFrontImg, trackBoxBottomImg
