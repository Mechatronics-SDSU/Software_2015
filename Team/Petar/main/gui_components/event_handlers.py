...
class EventHandlers:
    ...
    def guiKeyboardEvent(self, event):
        ...
        if chr(event.keycode) == "X" and not event.char == chr(24): #x controls toggling of different processing filters
            self.window.frontXZCounter += 1
            if self.window.frontXZCounter >= self.window.imagingFilterCount: #Amount of images I want to display on front processing image label
                self.window.frontXZCounter = self.window.imagingFilterCount  #Amount of images I want to display on front processing image label
            
        if chr(event.keycode) == "Z" and not event.char == chr(26): #z
            self.window.frontXZCounter -= 1
            if self.window.frontXZCounter <= 0:
                self.window.frontXZCounter = 0
          
        if event.char == chr(24): #Ctrl-x
            self.window.bottomXZCounter += 1
            if self.window.bottomXZCounter >= self.window.imagingFilterCount: #Amount of images I want to display on bottom processing image label
                self.window.bottomXZCounter = self.window.imagingFilterCount  #Amount of images I want to display on bottom processing image label
            
        if event.char == chr(26): #Ctrl-z
            self.window.bottomXZCounter -= 1
            if self.window.bottomXZCounter <= 0:
