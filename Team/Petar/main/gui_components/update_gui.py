...
camshiftCheckBoxPreviousState, rectangleCheckBoxPreviousState, circleCheckBoxPreviousState = 0, 0, 0
...
class UpdateGUI():
    ...
    def run(self):
        ...
        if self.window.spaceBar == False: #If space bar is pressed, scaledFrontImg and scaledBottomImg will maintain the same image which will seem to freeze the video stream
        ...
        #IMAGE PROCESSING       
        imageProcValues = self.__imageProcessing__(frontImg, bottomImg, self.window.frontRawImgLabel, self.window.bottomRawImgLabel, self.window.frontProcessedImgLabel, self.window.bottomProcessedImgLabel, self.window.filterScales, self.window.objectDetectScales)
    ...
    def __roiSample__(self, tolerance, frontRawImgLabel, bottomRawImgLabel, filterScales):
        ...
        if frontRawImgLabel.buttonReleased == True:
            ...
            filterScales[10].set(minHSV[0] - tolerance)
            filterScales[11].set(minHSV[1] - tolerance) 
            filterScales[12].set(minHSV[2] - tolerance) 
            filterScales[13].set(maxHSV[0] + tolerance) 
            filterScales[14].set(maxHSV[1] + tolerance) 
            filterScales[15].set(maxHSV[2] + tolerance)
    ...
    def __imageProcessing__(self, frontImg, bottomImg, frontRawImgLabel, bottomRawImgLabel, frontProcessedImgLabel, bottomProcessedImgLabel, filterScales, objectDetectScales):
        global track_window, camshiftCheckBoxPreviousState, rectangleCheckBoxPreviousState, circleCheckBoxPreviousState
        
        #INITILIZING OBJECT DETECTION VARIABLES EACH ITERATION
        trackBoxFrontImg, trackBoxBottomImg, rectEllipseFrontImg, rectEllipseBottomImg = None, None, [], []
        ...
        maskHSV3rdAxisFrontImg = maskFrontImg[:, :, numpy.newaxis]
        HSVBitwiseAndFrontImg = numpy.bitwise_and(scaledTabbedFrontImg, maskHSV3rdAxisFrontImg)
        
        #morphology filter
        morphologyFrontImg1 = cv2.morphologyEx(HSVBitwiseAndFrontImg, filterScales[6].get(), numpy.ones((5,5), numpy.uint8), iterations = filterScales[7].get())
        morphologyFrontImg2 = cv2.morphologyEx(morphologyFrontImg1, filterScales[8].get(), numpy.ones((5,5), numpy.uint8), iterations = filterScales[9].get())
        HSVFrontImg = cv2.cvtColor(morphologyFrontImg2, cv2.COLOR_BGR2HSV)
        lowerBound = (filterScales[0].get(), filterScales[1].get(), filterScales[2].get())
        upperBound = (filterScales[3].get(), filterScales[4].get(), filterScales[5].get())
        maskFrontImg = cv2.inRange(HSVFrontImg, lowerBound , upperBound) #Black and white image
        
        
        camshiftChecked = self.window.objectDetectScales_CheckBoxes[0].get()
        rectangleChecked = self.window.objectDetectScales_CheckBoxes[1].get()
        
        if camshiftCheckBoxPreviousState != camshiftChecked or rectangleCheckBoxPreviousState != rectangleChecked: #If the check boxed for object detect are changed, I need to put the default img to the processing label
            self.window.frontXZCounter = 0
            self.window.bottomXZCounter = 0
            
        camshiftCheckBoxPreviousState = camshiftChecked
        rectangleCheckBoxPreviousState = rectangleChecked
        
        #camshift detect
        if camshiftChecked == 1: #If camshift is checked
            trackBoxFrontImg, trackBoxWindowFrontImg = self.camshiftFront.camshift(morphologyFrontImg2, HSVFrontImg, maskFrontImg, self.processedImgWidth, self.processedImgHeight)
            if trackBoxFrontImg[0][0] != 0 and trackBoxFrontImg[0][1] != 0: #If their is something to track onto 
                cv2.putText(morphologyFrontImg2, "(" + str(trackBoxFrontImg[0][0]) + ", " + str(trackBoxFrontImg[0][1]) + ")", (int(trackBoxFrontImg[0][0]), int(trackBoxFrontImg[0][1])), cv2.FONT_HERSHEY_PLAIN, 1, (255, 255, 255))
                cv2.putText(morphologyFrontImg2, "(" + str(round(trackBoxFrontImg[1][0], 1)) + ", " + str(round(trackBoxFrontImg[1][1], 1)) + ")", (int(trackBoxFrontImg[0][0]), int(trackBoxFrontImg[0][1]+15)), cv2.FONT_HERSHEY_PLAIN, 1, (255, 255, 255))
                cv2.putText(morphologyFrontImg2, "(" + str(round(trackBoxFrontImg[2], 1)) + ")", (int(trackBoxFrontImg[0][0]), int(trackBoxFrontImg[0][1]+30)), cv2.FONT_HERSHEY_PLAIN, 1, (255, 255, 255))   
                
            
        #Rectangle Detect
        if camshiftChecked == 0 and rectangleChecked == 1: #If camshift is off but rectangle detect is on
            #Front
            grayFrontImg = cv2.cvtColor(morphologyFrontImg2, cv2.COLOR_BGR2GRAY)
            blurFrontImg = cv2.medianBlur(grayFrontImg, objectDetectScales[6].get()*2-1)
            cannyFrontImg = cv2.Canny(blurFrontImg, objectDetectScales[0].get(), objectDetectScales[3].get())
            returnImg, contours, hierarchy = cv2.findContours(cannyFrontImg, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
            
            for x in range(len(contours)):
                xRect,yRect,wRect,hRect = cv2.boundingRect(contours[x])
                
                widthHeightRequirement = wRect >= objectDetectScales[1].get() and hRect >= objectDetectScales[2].get() and wRect <= objectDetectScales[4].get() and hRect <= objectDetectScales[5].get()
                
                if widthHeightRequirement:
                    if self.window.frontXZCounter == 1:
                        cv2.drawContours(morphologyFrontImg2,[contours[x]],0,(0,0,255),2)
                        
                    try:
                        ellipse = cv2.fitEllipse(contours[x])
                        cv2.ellipse(morphologyFrontImg2, ellipse, (0,255,0), 2)
                        cv2.putText(morphologyFrontImg2, "(" + str(int(ellipse[0][0])) + ", "  + str(int(ellipse[0][1])) + ")", (int(ellipse[0][0]), int(ellipse[0][1])), cv2.FONT_HERSHEY_PLAIN, 1, (0, 255, 0))
                        rectEllipseFrontImg.append(((int(ellipse[0][0]), int(ellipse[0][1])), (int(ellipse[1][0]), int(ellipse[1][1])), int(ellipse[2])))
                    except:
                        pass
                                
        #Camshift and Rectangle Detect
        if camshiftChecked == 1 and rectangleChecked == 1: #If camshift and rectangle detect are on
            
            grayFrontImg = cv2.cvtColor(scaledTabbedFrontImg, cv2.COLOR_BGR2GRAY)
            blurFrontImg = cv2.medianBlur(grayFrontImg, objectDetectScales[6].get()*2-1)
            cannyFrontImg = cv2.Canny(blurFrontImg, objectDetectScales[0].get(), objectDetectScales[3].get())
            
            returnImg, contours, hierarchy = cv2.findContours(cannyFrontImg, cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE)
            
            trackBoxWindowX, trackBoxWindowY, trackBoxWindowWidth, trackBoxWindowHeight = trackBoxWindowFrontImg
            
            for x in range(len(contours)):
                xRect,yRect,wRect,hRect = cv2.boundingRect(contours[x])
                
                widthHeightRequirement = (wRect >= objectDetectScales[1].get() and hRect >= objectDetectScales[2].get() and wRect <= objectDetectScales[4].get() and hRect <= objectDetectScales[5].get()) and (wRect <= trackBoxWindowWidth and hRect <= trackBoxWindowHeight)
                locationRequirement = (xRect >= trackBoxWindowX and xRect+wRect <= trackBoxWindowX+trackBoxWindowWidth) and (yRect >= trackBoxWindowY and yRect+hRect <= trackBoxWindowY+trackBoxWindowHeight)
                
                if widthHeightRequirement and locationRequirement:
                    if self.window.frontXZCounter == 1:
                        cv2.drawContours(morphologyFrontImg2,[contours[x]],0,(0,0,255),2)
                    try:
                        ellipse = cv2.fitEllipse(contours[x])
        ...
        #PROCESSED IMG 1 TO TK IMG
        if (camshiftChecked == 1 and rectangleChecked == 1) or (camshiftChecked == 0 and rectangleChecked == 1):
            self.window.imagingFilterCount = 3
            
            if self.window.frontXZCounter == 0: #Changes the processed img feed to the filters previous to the one that is currently up
                bgrToRgbFrontImg = cv2.cvtColor(morphologyFrontImg2, cv2.COLOR_BGR2RGB)
                tkHSVFrontImg = PIL.ImageTk.PhotoImage(PIL.Image.fromarray(bgrToRgbFrontImg))
            
            elif self.window.frontXZCounter == 1: 
                bgrToRgbFrontImg = cv2.cvtColor(morphologyFrontImg2, cv2.COLOR_BGR2RGB)
                tkHSVFrontImg = PIL.ImageTk.PhotoImage(PIL.Image.fromarray(bgrToRgbFrontImg))
                
            elif self.window.frontXZCounter == 2:
                grayToRgbFrontImg = cv2.cvtColor(cannyFrontImg, cv2.COLOR_GRAY2RGB)
                tkHSVFrontImg = PIL.ImageTk.PhotoImage(PIL.Image.fromarray(grayToRgbFrontImg))
                
            elif self.window.frontXZCounter == 3:
                grayToRgbFrontImg = cv2.cvtColor(blurFrontImg, cv2.COLOR_GRAY2RGB)
                tkHSVFrontImg = PIL.ImageTk.PhotoImage(PIL.Image.fromarray(grayToRgbFrontImg))
                    
        else:
            self.window.imagingFilterCount = 0
        ...
        lowerBound = (filterScales[10].get(), filterScales[11].get(), filterScales[12].get())
        upperBound = (filterScales[13].get(), filterScales[14].get(), filterScales[15].get())
        ...
        #morphology filter
        morphologyBottomImg1 = cv2.morphologyEx(HSVBitwiseAndBottomImg, filterScales[16].get(), numpy.ones((5,5), numpy.uint8), iterations = filterScales[17].get())
        morphologyBottomImg2 = cv2.morphologyEx(morphologyBottomImg1, filterScales[18].get(), numpy.ones((5,5), numpy.uint8), iterations = filterScales[19].get())
        HSVBottomImg = cv2.cvtColor(morphologyBottomImg2, cv2.COLOR_BGR2HSV)
        lowerBound = (filterScales[10].get(), filterScales[11].get(), filterScales[12].get())
        upperBound = (filterScales[13].get(), filterScales[14].get(), filterScales[15].get())
        ...
        #camshift detect
        if camshiftChecked == 1: #If camshift is checked
            trackBoxBottomImg, trackBoxWindowBottomImg = self.camshiftBottom.camshift(morphologyBottomImg2, HSVBottomImg, maskBottomImg, self.processedImgWidth, self.processedImgHeight)
            if trackBoxBottomImg[0][0] != 0 and trackBoxBottomImg[0][1] != 0:
                cv2.putText(morphologyBottomImg2, "(" + str(trackBoxBottomImg[0][0]) + ", " + str(trackBoxBottomImg[0][1]) + ")", (int(trackBoxBottomImg[0][0]), int(trackBoxBottomImg[0][1])), cv2.FONT_HERSHEY_PLAIN, 1, (255, 255, 255))
                cv2.putText(morphologyBottomImg2, "(" + str(round(trackBoxBottomImg[1][0], 1)) + ", " + str(round(trackBoxBottomImg[1][1], 1)) + ")", (int(trackBoxBottomImg[0][0]), int(trackBoxBottomImg[0][1]+15)), cv2.FONT_HERSHEY_PLAIN, 1, (255, 255, 255))
                cv2.putText(morphologyBottomImg2, "(" + str(round(trackBoxBottomImg[2], 1)) + ")", (int(trackBoxBottomImg[0][0]), int(trackBoxBottomImg[0][1]+30)), cv2.FONT_HERSHEY_PLAIN, 1, (255, 255, 255)) 
                        
        #Rectangle Detect
        if camshiftChecked == 0 and rectangleChecked == 1: #If camshift is off but rectangle detect is on
            #Front
            grayBottomImg = cv2.cvtColor(morphologyBottomImg2, cv2.COLOR_BGR2GRAY)
            blurBottomImg = cv2.medianBlur(grayBottomImg, objectDetectScales[16].get()*2-1)
            cannyBottomImg = cv2.Canny(blurBottomImg, objectDetectScales[10].get(), objectDetectScales[13].get())
            returnImg, contours, hierarchy = cv2.findContours(cannyBottomImg, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
            
            for x in range(len(contours)):
                xRect,yRect,wRect,hRect = cv2.boundingRect(contours[x])
                
                widthHeightRequirement = wRect >= objectDetectScales[11].get() and hRect >= objectDetectScales[12].get() and wRect <= objectDetectScales[14].get() and hRect <= objectDetectScales[15].get()
                
                if widthHeightRequirement:
                    if self.window.bottomXZCounter == 1:
                        cv2.drawContours(morphologyBottomImg2,[contours[x]],0,(0,0,255),2)
                        
                    try:
                        ellipse = cv2.fitEllipse(contours[x])
                        cv2.ellipse(morphologyBottomImg2, ellipse, (0,255,0), 2)
                        cv2.putText(morphologyBottomImg2, "(" + str(int(ellipse[0][0])) + ", "  + str(int(ellipse[0][1])) + ")", (int(ellipse[0][0]), int(ellipse[0][1])), cv2.FONT_HERSHEY_PLAIN, 1, (0, 255, 0))
                        rectEllipseBottomImg.append(((int(ellipse[0][0]), int(ellipse[0][1])), (int(ellipse[1][0]), int(ellipse[1][1])), int(ellipse[2])))
                    except:
                        pass
        ...
        if camshiftChecked == 1 and rectangleChecked == 1: #If camshift and rectangle detect are on
            ...
            grayBottomImg = cv2.cvtColor(scaledTabbedBottomImg, cv2.COLOR_BGR2GRAY)
            blurBottomImg = cv2.medianBlur(grayBottomImg, objectDetectScales[16].get()*2-1)
            cannyBottomImg = cv2.Canny(blurBottomImg, objectDetectScales[10].get(), objectDetectScales[13].get())
            
            returnImg, contours, hierarchy = cv2.findContours(cannyBottomImg, cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE)
            
            trackBoxWindowX, trackBoxWindowY, trackBoxWindowWidth, trackBoxWindowHeight = trackBoxWindowBottomImg
            
            for x in range(len(contours)):
                xRect,yRect,wRect,hRect = cv2.boundingRect(contours[x])
                
                widthHeightRequirement = (wRect >= objectDetectScales[11].get() and hRect >= objectDetectScales[12].get() and wRect <= objectDetectScales[14].get() and hRect <= objectDetectScales[15].get()) and (wRect <= trackBoxWindowWidth and hRect <= trackBoxWindowHeight)
                locationRequirement = (xRect >= trackBoxWindowX and xRect+wRect <= trackBoxWindowX+trackBoxWindowWidth) and (yRect >= trackBoxWindowY and yRect+hRect <= trackBoxWindowY+trackBoxWindowHeight)
        ...
        if (camshiftChecked == 1 and rectangleChecked == 1) or (camshiftChecked == 0 and rectangleChecked == 1):
            ...
            if self.window.bottomXZCounter == 0: #Changes the processed img feed to the filters previous to the one that is currently up
                bgrToRgbBottomImg = cv2.cvtColor(morphologyBottomImg2, cv2.COLOR_BGR2RGB)
                tkHSVBottomImg = PIL.ImageTk.PhotoImage(PIL.Image.fromarray(bgrToRgbBottomImg))
            
            elif self.window.bottomXZCounter == 1: 
                bgrToRgbBottomImg = cv2.cvtColor(morphologyBottomImg2, cv2.COLOR_BGR2RGB)
                tkHSVBottomImg = PIL.ImageTk.PhotoImage(PIL.Image.fromarray(bgrToRgbBottomImg))
                
            elif self.window.bottomXZCounter == 2:
                grayToRgbBottomImg = cv2.cvtColor(cannyBottomImg, cv2.COLOR_GRAY2RGB)
                tkHSVBottomImg = PIL.ImageTk.PhotoImage(PIL.Image.fromarray(grayToRgbBottomImg))
                
            elif self.window.bottomXZCounter == 3:
                grayToRgbBottomImg = cv2.cvtColor(blurBottomImg, cv2.COLOR_GRAY2RGB)
                tkHSVBottomImg = PIL.ImageTk.PhotoImage(PIL.Image.fromarray(grayToRgbBottomImg))
        ...
        return trackBoxFrontImg, trackBoxBottomImg, rectEllipseFrontImg, rectEllipseBottomImg
